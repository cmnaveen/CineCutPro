...
- **2026-06-25** — **Phase 0 (Foundation) increment 2** — wired the new engine into the live app. `EditorContext` now runs undo/redo through `state/historyController.js`, which exposes two interchangeable backends — `snapshot` (the original ring) and **`patch`** (the Phase 0 command/inverse-patch core, now the default; set `localStorage ccp.historyMode='snapshot'` to revert). A new equivalence test (`state/historyController.test.js`) runs a 16-action real-reducer sequence through both backends and asserts identical undo/redo at every step. Added **typescript-eslint** (the core is now linted, not skipped) and a **GitHub Actions CI** workflow (`npm ci` → typecheck → test → build, required; lint advisory + a clean-lint gate on `src/core`). **59/59 tests**, typecheck + build green, dev-server boot verified. Note: a pre-existing `no-const-assign` crash in `clip/delete` (magnetic mode) at `editorReducer.js:402` was surfaced by the new TS-aware lint and flagged for a separate fix.
- **2026-06-25** — **Phase 0 (Foundation) increment 1** — see [`IMPLEMENTATION_PLAN.md`](IMPLEMENTATION_PLAN.md). Stood up the **pure edit-engine core** under `src/core/` (seed of `packages/engine`): **TypeScript** toolchain (`tsconfig.json`, `npm run typecheck`); **capability interfaces** + typed `CapabilityRegistry` with a first `WebStorage` impl wrapping the existing IndexedDB store/autosave (the web/native portability seam); and a **command + inverse-patch history engine** (`patch` diff/invert/apply — O(change) not O(project); `PatchHistory` with gesture coalescing; `CommandBus` with transactions) to replace snapshot undo. 30 new tests incl. an integration test that proves the patch engine reproduces the real `editorReducer`'s undo/redo — **55/55 green**, typecheck + production build green. App behavior unchanged (new core coexists; wiring into `EditorContext` is increment 2). ESLint temporarily skips `.ts` (typescript-eslint lands in increment 2).
